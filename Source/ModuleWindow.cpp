...
{
	App->gui->LogConsole(LOG("Init SDL window & surface"));
	bool ret = true;

	if(SDL_Init(SDL_INIT_VIDEO) < 0)
	{
		App->gui->LogConsole(LOG("SDL_VIDEO could not initialize! SDL_Error: %s\n", SDL_GetError()));
		ret = false;
	}
	else
	{
		...
		if(window == NULL)
		{
			App->gui->LogConsole(LOG("Window could not be created! SDL_Error: %s\n", SDL_GetError()));
			ret = false;
		}
		else
		{
...
{
	App->gui->LogConsole(LOG("Destroying SDL window and quitting all SDL systems"));
...
void ModuleWindow::GetMaxWindow(int& w, int& h)
{
	...
	if (SDL_GetDesktopDisplayMode(0, &dm) != 0)
	{
		if (App->gui != nullptr)
		{
			App->gui->LogConsole(LOG("SDL_GetDesktopDisplayMode failed: %s", SDL_GetError()));
		}			
	}
	else
	{
